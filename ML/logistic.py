    ...
    with open(csv_file, 'r') as csv:
        ...
        for i, line in enumerate(lines):
            ...
            if test_file:
                ...
                tweets.append((tweet_id, int(sentiment), feature_vector)) #int(sentiment)
...
def build_model():
    model = Sequential()
    #model.add(Dense(5,input_dim=VOCAB_SIZE,activation='softmax'))
    model.add(Dense(3, input_dim=VOCAB_SIZE, activation='sigmoid'))
...
if __name__ == '__main__':
    ...
    best_val_acc = 0.0
    #tweets = process_tweets(TRAIN_PROCESSED_FILE, test_file=False) comment it
    #train_tweets, val_tweets = utils.split_data(tweets)            comment it
    for j in range(nb_epochs):
        ...
        for training_set_X, training_set_y in extract_features(train_tweets, feat_type=FEAT_TYPE, batch_size=batch_size, test_file=False):
            training_set_y = to_categorical(training_set_y) # new line added
            #print(training_set_y.shape)
            #o = model.train_on_batch(training_set_X, to_categorical(training_set_y)) #added categorical here
            o = model.train_on_batch(training_set_X, training_set_y)
            #o = model.train_on_batch(training_set_X, training_set_y)  # added categorical here
    ...
    for test_set_X, _ in extract_features(test_tweets, feat_type=FEAT_TYPE, batch_size=batch_size, test_file=True):
        prediction = model.predict_on_batch(test_set_X)
        k = 0
        for x in prediction:
            #print(x, labels1[np.argmax(x)])
            item = labels1[np.argmax(x)]
            pred.append(item)
        #print(type(pred))
